...
public class UserSettingsData implements Serializable {
		...
		try {
			...
			if (locale == null) {
				...
				if (locale == null || !getSupportedLocales(fc).contains(locale)) {
					// No header for "accept-language" or not suppoted Locale
					// ==> take default locale as fallback
	...
	public void saveSettings(AjaxBehaviorEvent e) {
		...
		fc.getApplication().getNavigationHandler().handleNavigation(fc, "null",
				fc.getViewRoot().getViewId() + "?faces-redirect=true");
	}
		...
		if (locales == null) {
			...
			while (iter.hasNext()) {
				...
				locales.add(new SelectItem(supLocale,
						msgs.getText("jsftoolkit.locale." + LocaleConverter.getLocaleString(supLocale, '_'))));
			}
		}
	...
	public Locale getLocale() {
		Locale locale = (Locale) settings.get(LOCALE_KEY);
		if (locale == null) {
			return localDefault;
		} else {
			return locale;
